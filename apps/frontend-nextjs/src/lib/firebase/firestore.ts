  ...
  static async saveConversation(conversation: FirestoreConversation): Promise<FirebaseApiResponse<string>> {
    try {
      ...
      if (conversation.id) {
        ...
        const docRef = doc(db, this.collection, conversation.id);
...
export class FeedbackRepository {
  ...
  static async saveFeedback(feedback: Omit<FirestoreFeedback, 'id' | 'createdAt'>): Promise<FirebaseApiResponse<string>> {
    try {
      ...
      const docRef = await addDoc(collection(db, this.collection), feedbackWithTimestamps);
      ...
      for (const item of localStorageData) {
        try {
          ...
          if (item.type === 'conversation') {
            const docRef = doc(collection(db, FIRESTORE_CONFIG.COLLECTIONS.CONVERSATIONS));
            batch.set(docRef, {
              ...item.data,
              userId,
              ...createTimestamps(),
            ...
            const docRef = doc(db, FIRESTORE_CONFIG.COLLECTIONS.USER_PROFILES, userId);
            batch.set(docRef, {
              ...item.data,
              uid: userId,
              ...createTimestamps(),
...
export class RealtimeSubscriptions {
  ...
  static subscribeToUserConversations(
    ...
    const q = query(
      collection(db, FIRESTORE_CONFIG.COLLECTIONS.CONVERSATIONS),
      where('userId', '==', userId),
      orderBy('lastActivity', 'desc'),
