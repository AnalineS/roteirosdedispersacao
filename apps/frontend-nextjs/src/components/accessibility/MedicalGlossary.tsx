...
export default function MedicalGlossary({ 
  ...
  const filteredTerms = medicalTerms.filter(medicalTerm => {
    const matchesSearch = searchTerm === '' || 
      medicalTerm.technical.toLowerCase().includes(searchTerm.toLowerCase()) ||
      medicalTerm.simple.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesCategory = selectedCategory === 'all' || medicalTerm.category === selectedCategory;
    
    return matchesSearch && matchesCategory;
  }).slice(0, maxResults);

  // Encontrar termo específico
  const specificTerm = medicalTerms.find(t => 
    ...
    >
      ...
      {showSearch && (
        <div style={{ marginBottom: modernChatTheme.spacing.lg }}>
          <label htmlFor="medical-search" style={{
            display: 'block',
            marginBottom: modernChatTheme.spacing.sm,
            fontWeight: '600',
      ...
      <div role="list" aria-label={`${filteredTerms.length} termos encontrados`}>
        {filteredTerms.length === 0 ? (
          ...
          filteredTerms.map((medicalTerm) => (
            ...
            >
              ...
              <p style={{
                margin: 0,
                marginBottom: medicalTerm.example ? modernChatTheme.spacing.sm : 0,
                fontSize: '16px',
                lineHeight: '1.6',
                color: modernChatTheme.colors.neutral.text
              }}>
...
export function GlossaryTerm({ children, term }: { children: React.ReactNode; term: string }) {
  return (
