...
export type GlobalAction =
  ...
  | {
      ...
      payload: { module: string; completion: number };
    }
...
export const GlobalContextProvider: React.FC<GlobalContextProviderProps> = ({
  ...
  // Note: useTrackingContext must be called unconditionally
        ...
        if (data.completionStatus) {
          Object.entries(data.completionStatus).forEach(
            ([module, completion]) => {
              dispatch({
                type: "UPDATE_COMPLETION",
                payload: { module, completion: completion as number },
              });
            },
          );
        }
  ...
  const helpers = useMemo(
    () => ({
      ...
      navigate: (path: string) =>
        dispatch({
          type: "NAVIGATE",
          payload: { path, previousPath: state.navigation.currentPath },
        }),
  ...
  const computed = useMemo(
    () => ({
      isModuleCompleted: (module: string) =>
        (state.completionStatus[module] || 0) >= 1,
      getCompletionRate: () => {
        const total = Object.keys(state.completionStatus).length;
        const completed = Object.values(state.completionStatus).filter(
          (c) => c >= 1,
        ).length;
      ...
      isAccessibilityEnabled:
        ...
        state.accessibility.contrast === "high" ||
        state.accessibility.fontSize !== "medium",
    }),
...
export const useGlobalErrors = () => {
  const { state, addError, clearErrors, hasErrors, criticalErrors } =
    useGlobalContext();
  return {
    errors: state.errors,
    addError,
...
function formatPathSegment(segment: string): string {
  ...
  return (
    formatMap[segment] || segment.charAt(0).toUpperCase() + segment.slice(1)
  );
}
