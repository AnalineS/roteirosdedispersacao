...
class AIProviderManager:
    ...
    def _initialize_models(self):
        ...
        if not self.models:
            logger.warning("⚠️ Nenhuma API key configurada - apenas fallbacks disponíveis")
        else:
            logger.info(f"🎯 {len(self.models)} modelos configurados")
    ...
    def _record_success(self, provider: str, response_time: float):
        ...
        logger.debug(f"✅ {provider} success - {response_time:.2f}s")
    ...
    def _record_failure(self, provider: str, error: str):
        ...
        if provider in self.circuit_breakers:
            ...
            if breaker.failure_count >= breaker.failure_threshold:
                if breaker.state != CircuitBreakerState.OPEN:
                    ...
                    logger.warning(f"⚠️ Circuit breaker {provider}: -> OPEN")
        ...
        logger.warning(f"❌ {provider} failure: {error}")
    ...
    async def generate_response(
        ...
        logger.warning("⚠️ Todos os modelos falharam - usando fallback")
        return self._generate_fallback_response(messages), {
            'model_used': 'fallback',
            'provider': 'internal',
            'success': False,
