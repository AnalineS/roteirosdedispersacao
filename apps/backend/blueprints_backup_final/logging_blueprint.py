...
def delete_user_data():
    ...
    try:
        ...
        # IMPLEMENTAÇÃO REAL DE DELEÇÃO LGPD
        deletion_result = {
            "deleted_records": {
                "database": 0,
                "cloud_logs": 0,
        ...
        # 1. Deletar dados do usuário do banco SQLite
        try:
            import sqlite3
            import os
            ...
            databases = [
                'data/database/roteiros_dispensacao.db',
            ...
            for db_path in databases:
                if os.path.exists(db_path):
                    ...
                    # Deletar dados do usuário de todas as tabelas
                    tables = cursor.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()

                    for table in tables:
                        table_name = table[0]
                        # Verificar se a tabela tem coluna user_id ou uid
                        columns = cursor.execute(f"PRAGMA table_info({table_name})").fetchall()
                        user_columns = [col[1] for col in columns if col[1] in ['user_id', 'uid', 'userId']]

                        for col in user_columns:
                            cursor.execute(f"DELETE FROM {table_name} WHERE {col} = ?", (user_id,))
                            deletion_result["deleted_records"]["database"] += cursor.rowcount

                    conn.commit()
                    conn.close()

            logger.info(f"✅ LGPD: Deletados {deletion_result['deleted_records']['database']} registros do banco")

        except Exception as e:
            logger.error(f"Erro ao deletar dados do banco: {e}")
            deletion_result["status"] = "partial"

        # 2. Deletar logs do Cloud Logging (se disponível)
        try:
            from google.cloud import logging as cloud_logging_lib
            import os

            if os.getenv('GCP_PROJECT_ID'):
                client = cloud_logging_lib.Client()

                # Filtro para logs do usuário
                filter_str = f'jsonPayload.user_id="{user_id}" OR jsonPayload.uid="{user_id}"'

                # Listar e deletar entradas
                for entry in client.list_entries(filter_=filter_str):
                    entry.delete()
                    deletion_result["deleted_records"]["cloud_logs"] += 1

                logger.info(f"✅ LGPD: Deletados {deletion_result['deleted_records']['cloud_logs']} logs do Cloud")
        except Exception as e:
            logger.warning(f"Cloud Logging não disponível ou erro: {e}")

        # 3. Limpar cache local e arquivos temporários
        try:
            import glob
            import os
            ...
            for pattern in patterns:
                for file_path in glob.glob(pattern):
                    os.remove(file_path)
                    deletion_result["deleted_records"]["local_storage"] += 1

            logger.info(f"✅ LGPD: Deletados {deletion_result['deleted_records']['local_storage']} arquivos locais")

        except Exception as e:
            logger.warning(f"Erro ao limpar arquivos locais: {e}")

        # 4. Notificar sistemas integrados (Supabase para RAG)
        try:
            from services.rag.supabase_rag_system import SupabaseRAGSystem
            rag = SupabaseRAGSystem()
            if hasattr(rag, 'delete_user_data'):
                rag.delete_user_data(user_id)
                logger.info(f"✅ LGPD: Dados removidos do sistema RAG")
        except Exception as e:
            logger.warning(f"Erro ao notificar sistema RAG: {e}")
