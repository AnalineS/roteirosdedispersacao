...
def get_system_info() -> Dict[str, Any]:
    """Coleta informações do sistema com otimizações de memória"""
    try:
        ...
        if PSUTIL_AVAILABLE:
            ...
            system_info.update({
                "memory": {
                    ...
                    "percent": mem.percent,
                    "status": (
                        "critical" if mem.percent >= 85 else
                        "warning" if mem.percent >= 75 else
                        "good" if mem.percent >= 50 else "excellent"
                    )
                },
                "disk": {
            ...
            # Adicionar alerta médico se memória crítica
            if mem.percent >= 85:
                system_info["memory"]["alert"] = {
                    "level": "CRÍTICO",
                    "message": f"ALERTA MÉDICO [high_memory_usage]: {mem.percent:.1f}%",
                    "recommendation": "Execute otimização emergencial via /api/v1/memory/optimize/force"
                }
        else:
            system_info.update({
        ...
        # Adicionar informações do otimizador de memória se disponível
        try:
            from core.performance.memory_optimizer import get_memory_optimizer
            memory_optimizer = get_memory_optimizer()
            memory_stats = memory_optimizer.get_memory_stats()

            system_info["memory_optimization"] = {
                "enabled": True,
                "current_status": memory_stats.get("current", {}).get("status", "unknown"),
                "cache_items": memory_stats.get("cache", {}).get("items", 0),
                "gc_collections": memory_stats.get("optimization", {}).get("gc_collections", 0)
            }
        except Exception:
            system_info["memory_optimization"] = {"enabled": False}
