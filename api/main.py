...
# Importar sistemas otimizados - TODAS AS FUNCIONALIDADES ATIVADAS
try:
    ...
    print(f"⚠️ Advanced features disabled, using fallback: {e}")
...
def answer_question(question, persona):
    ...
    try:
        ...
        # Gerar resposta usando sistemas avançados
        if ADVANCED_FEATURES:
            # Usar sistemas enhanced com prompts otimizados
            if persona == 'dr_gasnelio':
                prompt = get_enhanced_dr_gasnelio_prompt(question)
                ai_response = get_free_ai_response(question, persona, context)
                if ai_response:
                    validation = validate_dr_gasnelio_response(ai_response, question)
                    logger.info(f"Qualidade Dr. Gasnelio: {validation['quality_score']:.0f}%")
            elif persona == 'ga':
                prompt = get_enhanced_ga_prompt(question)
                ai_response = get_free_ai_response(question, persona, context)
                if ai_response:
                    validation = validate_ga_response(ai_response, question)
                    logger.info(f"Empatia Gá: {validation['empathy_score']:.0f}%")
            else:
                ai_response = get_free_ai_response(question, persona, context)
...
def index():
    ...
    try:
        ...
        logger.warning(f"Frontend não encontrado, servindo API info: {e}")
        return jsonify({
