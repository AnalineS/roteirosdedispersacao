...
{'🎉 **SISTEMA APROVADO** - Todas as categorias atingiram os critérios mínimos' if results['overall_success'] else '[WARNING] **SISTEMA NECESSITA CORREÇÕES** - Uma ou mais categorias abaixo do critério'}
...
## [TARGET] Critérios de Aprovação Científica

### Critérios Mínimos Atingidos:
{'[OK]' if results['dosage_accuracy'] >= 80 else '[ERROR]'} **Precisão de Dosagens:** ≥ 80% (Atual: {results['dosage_accuracy']:.1f}%)
{'[OK]' if results['content_completeness'] >= 70 else '[ERROR]'} **Completude de Conteúdo:** ≥ 70% (Atual: {results['content_completeness']:.1f}%)
{'[OK]' if results['safety_info'] >= 70 else '[ERROR]'} **Informações de Segurança:** ≥ 70% (Atual: {results['safety_info']:.1f}%)
{'[OK]' if results['contraind_accuracy'] >= 70 else '[ERROR]'} **Precisão de Contraindicações:** ≥ 70% (Atual: {results['contraind_accuracy']:.1f}%)
