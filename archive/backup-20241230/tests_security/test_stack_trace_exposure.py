...
class TestStackTraceExposurePrevention:
    ...
    def test_configuration_error_sanitization(self):
        ...
        for component, expected_generic in test_cases:
            with patch('main.logger') as mock_logger:
                ...
                mock_logger.warning(f"[WARNING] Erro ao inicializar {component} [Exception]: Configuração indisponível")
    ...
    def test_startup_error_sanitization(self):
        ...
        with patch('main.logger') as mock_logger:
            ...
            mock_logger.error(f"[ERROR] Erro ao iniciar servidor [{error_type}]: Falha na inicialização")
...
class TestSecurityComplianceValidation:
    ...
    def test_cwe_209_information_exposure_prevention(self):
        ...
        error_messages = [
            ...
            "[ERROR] Erro ao carregar Fallback Inteligente: ImportError", 
            "[WARNING] Erro ao inicializar Security Middleware [Exception]: Configuração indisponível",
            "[WARNING] Erro ao configurar JWT [Exception]: Configuração de autenticação indisponível"
        ]
    ...
    def test_cwe_497_information_exposure_prevention(self):
        ...
        log_messages = [
            "[ERROR] Erro ao iniciar servidor [Exception]: Falha na inicialização",
            "[WARNING] Erro ao inicializar otimizações [Exception]: Recursos avançados indisponíveis"
        ]
