...
{'🎉 **EXCELENTE** - Sistema detecta limitações adequadamente' if results['success_rate'] >= 90 else '✅ **BOA** - Sistema funciona com pequenos ajustes' if results['success_rate'] >= 80 else '⚠️ **NECESSITA MELHORIAS** - Falhas críticas de detecção'}
...
- Doenças diferentes: {'✅ Detectado' if results['out_of_scope']['diseases_detected'] else '❌ Não detectado'}
- Medicina geral: {'✅ Detectado' if results['out_of_scope']['general_med_detected'] else '❌ Não detectado'}
- Assuntos não-médicos: {'✅ Detectado' if results['out_of_scope']['non_medical_detected'] else '❌ Não detectado'}
- Medicamentos diferentes: {'✅ Detectado' if results['out_of_scope']['other_drugs_detected'] else '❌ Não detectado'}
...
**Tipos de Perguntas Testadas:**
- Conceitos básicos: {'✅ Mantido' if results['in_scope']['basic_concepts_ok'] else '❌ Problemas'}
- Composição PQT-U: {'✅ Mantido' if results['in_scope']['composition_ok'] else '❌ Problemas'}
- Orientações práticas: {'✅ Mantido' if results['in_scope']['practical_ok'] else '❌ Problemas'}
- Populações especiais: {'✅ Mantido' if results['in_scope']['special_pop_ok'] else '❌ Problemas'}
...
**Principais Desafios:**
- Medicamentos compartilhados: {'✅ Adequado' if results['edge_cases']['shared_drugs_ok'] else '❌ Inadequado'}
- Usos off-label: {'✅ Adequado' if results['edge_cases']['off_label_ok'] else '❌ Inadequado'}
- Diagnóstico diferencial: {'✅ Adequado' if results['edge_cases']['differential_ok'] else '❌ Inadequado'}
- Comorbidades: {'✅ Adequado' if results['edge_cases']['comorbidities_ok'] else '❌ Inadequado'}
...
{'✅' if results['redirects_adequately'] else '❌'} **Redirecionamento Adequado:** Sistema orienta buscar profissional quando necessário
{'✅' if results['maintains_personality'] else '❌'} **Manutenção de Personalidade:** Personas mantêm coerência ao detectar limitações
{'✅' if results['avoids_invention'] else '❌'} **Evita Invenção:** Sistema não inventa informações fora do escopo
{'✅' if results['scope_consistency'] else '❌'} **Consistência de Escopo:** Detecção consistente entre diferentes formulações
