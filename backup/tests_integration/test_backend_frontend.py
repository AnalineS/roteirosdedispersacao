...
class IntegrationTester:
    ...
    def test_health_check(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log(f"[OK] Backend online - Status: {data.get('status')}")
                ...
                self.log(f"[ERROR] Health check falhou - Status: {response.status_code}", "ERROR")
                self.test_results['failed'] += 1
                self.test_results['errors'].append(f"Health check retornou {response.status_code}")
                return False
                
        except Exception as e:
            self.log(f"[ERROR] Erro no health check: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_personas_endpoint(self) -> bool:
        ...
        self.log("[SEARCH] Teste 2: Endpoint de Personas")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_personas:
                    self.log(f"[ERROR] Personas faltando: {missing_personas}", "ERROR")
                    self.test_results['failed'] += 1
                    self.test_results['errors'].append(f"Personas faltando: {missing_personas}")
                    return False
                
                self.log(f"[OK] Personas carregadas: {list(personas.keys())}")
                ...
                for persona_id, persona_data in personas.items():
                    ...
                    if missing_fields:
                        self.log(f"[ERROR] Persona {persona_id} com campos faltando: {missing_fields}", "ERROR")
                        self.test_results['failed'] += 1
                ...
                self.log(f"[ERROR] Endpoint personas falhou - Status: {response.status_code}", "ERROR")
                self.test_results['failed'] += 1
                self.test_results['errors'].append(f"Personas endpoint retornou {response.status_code}")
                return False
                
        except Exception as e:
            self.log(f"[ERROR] Erro no endpoint personas: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_chat_endpoint(self) -> bool:
        ...
        self.log("[SEARCH] Teste 3: Endpoint de Chat")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            for i, test_case in enumerate(test_questions, 1):
                ...
                if response.status_code == 200:
                    ...
                    self.log(f"   [OK] Resposta recebida - Persona: {persona}, Confiança: {confidence:.2f}")
                    ...
                    if len(found_keywords) >= 2:  # Pelo menos 2 palavras-chave
                        self.log(f"   [OK] Palavras-chave encontradas: {found_keywords}")
                    else:
                        self.log(f"   [WARNING]  Poucas palavras-chave encontradas: {found_keywords}", "WARN")
                    
                    # Verificar metadados
                    if 'request_id' in data and 'processing_time_ms' in data:
                        processing_time = data['processing_time_ms']
                        self.log(f"   [OK] Metadados OK - Tempo: {processing_time}ms")
                    else:
                        self.log(f"   [WARNING]  Metadados incompletos", "WARN")
                    
                else:
                    self.log(f"   [ERROR] Chat falhou - Status: {response.status_code}", "ERROR")
            ...
            if all_passed:
                ...
                self.log("[OK] Todos os testes de chat passaram")
                return True
            else:
                self.test_results['failed'] += 1
            ...
            self.log(f"[ERROR] Erro no endpoint chat: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_scope_endpoint(self) -> bool:
        ...
        self.log("[SEARCH] Teste 4: Endpoint de Escopo")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_sections:
                    self.log(f"[ERROR] Seções faltando no escopo: {missing_sections}", "ERROR")
                    self.test_results['failed'] += 1
                    return False
                
                self.log("[OK] Informações de escopo carregadas")
                ...
                if scope_response.status_code == 200:
                    ...
                    self.log(f"[OK] Análise de escopo - No escopo: {analysis.get('is_in_scope')}")
                    ...
                    self.log(f"[ERROR] POST scope falhou - Status: {scope_response.status_code}", "ERROR")
                    self.test_results['failed'] += 1
                    return False
            else:
                self.log(f"[ERROR] GET scope falhou - Status: {response.status_code}", "ERROR")
                self.test_results['failed'] += 1
                return False
                
        except Exception as e:
            self.log(f"[ERROR] Erro no endpoint scope: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_feedback_endpoint(self) -> bool:
        ...
        self.log("[SEARCH] Teste 5: Endpoint de Feedback")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log(f"[OK] Feedback enviado - ID: {data.get('request_id')}")
                ...
                self.log(f"[ERROR] Feedback falhou - Status: {response.status_code}", "ERROR")
                self.test_results['failed'] += 1
                return False
                
        except Exception as e:
            self.log(f"[ERROR] Erro no endpoint feedback: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_stats_endpoint(self) -> bool:
        ...
        self.log("[SEARCH] Teste 6: Endpoint de Estatísticas")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_sections:
                    self.log(f"[ERROR] Seções faltando nas stats: {missing_sections}", "ERROR")
                    self.test_results['failed'] += 1
                    return False
                
                self.log("[OK] Estatísticas do sistema carregadas")
                ...
                self.log(f"[ERROR] Stats falhou - Status: {response.status_code}", "ERROR")
                self.test_results['failed'] += 1
                return False
                
        except Exception as e:
            self.log(f"[ERROR] Erro no endpoint stats: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_error_handling(self) -> bool:
        ...
        self.log("[SEARCH] Teste 7: Tratamento de Erros")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            for test in error_tests:
                ...
                if response.status_code == test['expected_status']:
                    self.log(f"   [OK] Erro tratado corretamente - Status: {response.status_code}")
                    
                    # Verificar se retorna JSON de erro válido
                    try:
                        error_data = response.json()
                        if 'error' in error_data:
                            self.log(f"   [OK] JSON de erro válido")
                        else:
                            self.log(f"   [WARNING]  JSON de erro sem campo 'error'", "WARN")
                    except:
                        self.log(f"   [WARNING]  Resposta não é JSON válido", "WARN")
                else:
                    self.log(f"   [ERROR] Status inesperado: {response.status_code} (esperado: {test['expected_status']})", "ERROR")
                    all_passed = False
            ...
            if all_passed:
                ...
                self.log("[OK] Todos os testes de erro passaram")
                return True
            else:
                self.test_results['failed'] += 1
            ...
            self.log(f"[ERROR] Erro no teste de tratamento de erros: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def test_rate_limiting(self) -> bool:
        ...
        self.log("[SEARCH] Teste 8: Rate Limiting")
        self.test_results['total_tests'] += 1
        ...
        try:
            ...
            for i in range(35):  # Mais que o limite de 30/min
                ...
                if response.status_code == 429:
                    self.log(f"   [OK] Rate limit ativado na requisição {i+1}")
                    rate_limit_triggered = True
                    
                    # Verificar headers de rate limit
                    if 'X-RateLimit-Limit' in response.headers:
                        self.log(f"   [OK] Headers de rate limit presentes")
                    
                    break
                elif response.status_code != 200:
                    self.log(f"   [ERROR] Erro inesperado: {response.status_code}", "ERROR")
                    self.test_results['failed'] += 1
                    return False
            ...
            if rate_limit_triggered:
                ...
                self.log("[OK] Rate limiting funcionando")
                return True
            else:
                self.log("[WARNING]  Rate limiting não foi ativado (pode estar configurado com limite alto)", "WARN")
                self.test_results['passed'] += 1  # Não é necessariamente um erro
                return True
                
        except Exception as e:
            self.log(f"[ERROR] Erro no teste de rate limiting: {e}", "ERROR")
            self.test_results['failed'] += 1
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.log("[START] Iniciando Testes de Integração Backend-Frontend")
        ...
        for test_func in tests:
            try:
                ...
                self.log(f"[ERROR] Erro crítico no teste {test_func.__name__}: {e}", "ERROR")
                self.test_results['failed'] += 1
        ...
        self.log(f"[OK] Sucessos: {self.test_results['passed']}")
        self.log(f"[ERROR] Falhas: {self.test_results['failed']}")
        self.log(f"[REPORT] Taxa de sucesso: {self.test_results['success_rate']}%")
        ...
        if self.test_results['success_rate'] >= 90:
            ...
            self.log("\n[WARNING]  INTEGRAÇÃO BACKEND-FRONTEND: PARCIALMENTE FUNCIONAL")
        else:
            self.log("\n[ALERT] INTEGRAÇÃO BACKEND-FRONTEND: CRÍTICA - NECESSITA CORREÇÃO")
...
def main():
    ...
    try:
        ...
        print(f"\n[WARNING]  Erro ao salvar resultados: {e}")
